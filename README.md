...
- **Chunked large-file upload** — the browser uploads the file in small chunks
  ...
  stopped. On Chrome/Edge over HTTPS you can instead tick **"Keep uploading after I
  close this page"** to hand the upload to the browser's **Background Fetch** (via a
  service worker), so it continues even after the tab is closed and the file appears
  in the picker when it finishes. (A URL import or the Transfers page remains the
  fully server-side unattended route.)
...
Tell a Moodle-generated page apart from a proxy-generated one: a Moodle page
starts with `<!DOCTYPE html>`, whereas a bare `<html>` followed by
`<head><title>NNN …` is a page generated by nginx (or a reverse proxy) itself. A
proxy page does **not** mean PHP was never involved — for a 502 or 504 the
request usually did reach PHP-FPM, which then timed out, crashed or closed the
connection, so read the PHP-FPM log as well as nginx's.
...
|---|---|---|
| **504** (or `upstream timed out`) | Copying the staged file into the file area took longer than the proxy/PHP timeout. | Raise `fastcgi_read_timeout` (or `proxy_read_timeout` when proxying) in nginx, `request_terminate_timeout` in the PHP-FPM pool, and `max_execution_time` in `php.ini`. Set them comfortably above the time it takes to copy your largest file. (`send_timeout` does not apply — it bounds writing the response to the browser, not waiting on PHP.) |
| **502 Bad Gateway** | nginx could not get a valid response from PHP-FPM — the pool being down, a socket/connection failure, a killed worker, or an out-of-memory kill are all possible. | Read the nginx **and** PHP-FPM error logs first and fix the cause they name. The file-pool copy streams to disk, so a bigger file does not by itself need proportionally more PHP `memory_limit`; raise `memory_limit` only if the log shows an OOM. |
| **500**, or a **200** whose body is HTML beginning with `<div class="… debuggingmessage">` | A PHP error or debugging output leaked into the JSON reply, which breaks every insert regardless of file size. | Turn off **Display debug messages** (Site administration → Development → Debugging) so the reply is clean JSON, but keep a logged debug level until you have captured and fixed the underlying warning in the log. Lower the level to *NONE* only afterwards, if that is your production policy. |

A failing **chunked upload** is a different problem from this insert error. While
the large-file dialogue is still uploading, its requests go to `upload_ajax.php`
and the dialogue reports any error (including a 413) itself — they do not travel
through `filepicker.js`, so they never produce the `Y.JSON.parse` failure above.
If big uploads fail behind a proxy that rejects large request bodies, raise nginx
`client_max_body_size` or lower the **Chunk size** setting (see [Settings](#settings));
neither affects the picker-insert response covered here.
