define("repository_largefile/upload", ["exports", "core/pubsub", "core/modal_save_cancel", "core/modal_events", "core/templates", "core/str", "core/notification", "core/config"], function (_exports, _pubsub, _modal_save_cancel, _modal_events, _templates, _str, _notification, config) {
  ...
  const bgRecordsForContext = contextId => {
    const map = readBgMap();
    const now = Date.now();
    return Object.keys(map).filter(token => map[token] && map[token].contextId === contextId && now - (map[token].updated || 0) <= RESUME_BG_TTL_MS).map(token => Object.assign({
      token: token
    }, map[token])).sort((a, b) => (b.updated || 0) - (a.updated || 0));
  };
  ...
  const resumeBackgroundUpload = async (file, token, chunkSize, onProgress, controller) => {
    const statusResult = await postRequest({
      action: 'bgstatus',
      id: token
    }, null, null, null, controller);
    if (controller.cancelled) {
    ...
    if (statusResult.status !== 200 || snap === null || snap.error !== undefined || !Array.isArray(snap.missing)) {
      throw new Error(snap && snap.error ? snap.error : await (0, _str.getString)('erroruploadfailed', 'repository_largefile'));
    }
  ...
  const openUploadModal = async data => {
    ...
    const maybeOfferResume = async () => {
      ...
      if (fg) {
        records.push(Object.assign({}, fg, {
          background: false
        }));
      }
      bgRecordsForContext(data.contextId).forEach(rec => records.push(Object.assign({}, rec, {
        background: true
      })));
      if (!records.length) {
        hideResume();
    ...
    root.on(_modal_events.default.shown, async () => {
      ...
      if (input) {
        input.addEventListener('change', async () => {
          ...
          const match = selectedFile ? resumeCandidates.find(c => c.filename === selectedFile.name && c.size === selectedFile.size && c.fingerprint === fingerprint) : null;
          if (match) {
            pendingResume = match;
            resumeActive = true;
    ...
    root.on(_modal_events.default.save, async e => {
      ...
      try {
        ...
        if (urlTabActive) {
          ...
          hideResume();
          setStatus(await (0, _str.getString)('uploading', 'repository_largefile'));
          staged = backgroundResume ? await resumeBackgroundUpload(selectedFile, tokenId, chunkSize, setProgress, controller) : await uploadFileChunked(selectedFile, tokenId, chunkSize, setProgress, controller, resumeFrom);
        }
