...
$string['completeduploadgone'] = 'That completed upload is no longer available — the cleanup task purged it, another admin removed it or already sent it to a destination, or its row was removed manually. Refresh the Transfers page to see the current list.';
$string['completeduploadnofile'] = 'That completed upload\'s row is here but its file is missing on disk — the file was removed outside the plugin (a manual rm on the chunk area, a filesystem snapshot restore) or a row-delete failed while the file went. Remove the row from Completed uploads to clear it.';
